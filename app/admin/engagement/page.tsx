...
export default function EngagementAnnouncementsPage() {
  const { toast } = useToast();
  const [open, setOpen] = React.useState(false);
  const [announcements, setAnnouncements] = React.useState<Announcement[]>(initialAnnouncements);
  const [form, setForm] = React.useState({ title: '', category: 'Company', content: '', pinned: false });
  ...
  const handleSubmit = () => {
    ...
    setForm({ title: '', category: 'Company', content: '', pinned: false });
    setOpen(false);
  ...
  return (
    <div>
      <PageHeader title="Announcements" description="Share company news and updates with all employees." breadcrumbs={[{ label: 'Admin', href: '/admin/dashboard' }, { label: 'Engagement' }, { label: 'Announcements' }]} action={{ label: 'New announcement', icon: 'Plus', onClick: () => setOpen(true) }} />
